...
type Host struct {
	Id         int       `orm:"column(uuid);pk"`
	Ip         string    `orm:"column(ip);size(15);null"`
	Hostname   string    `orm:"column(hostname);size(256);null"`
	CreateTime time.Time `orm:"column(create_time);type(timestamp);null;auto_now_add"`
	UpdateTime time.Time `orm:"column(update_time);type(timestamp);null;auto_now_add"`
}
